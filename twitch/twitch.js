	...
	function getStreamerData() {
		async function getUserData() {
			for(let i = 0; i < streamers.length; i++) {
				let res = await fetch(`https://twitch-proxy.freecodecamp.rocks/twitch-api/users/${streamers[i].name}`);
				const userData = await res.json();
				res = await fetch(`https://twitch-proxy.freecodecamp.rocks/twitch-api/channels/${streamers[i].name}`);
				...
				if(channelData.status.length > 50) {
					...
					streamers[i].status = channelData.status.split('').join('');
				}
			}
		}
